...
from game.game import (
    Game, MovementDirection, TILE_SIZE, GRAVITY, JUMP_STRENGTH, 
    MOVE_SPEED, MAX_FALLING_SPEED
)
...
class TestGameInitialization:
    ...
    def test_initialize_game_with_config(self, game_simple):
        ...
        assert game_simple.config == Config("maps/test_simple.txt", 2, 1500)
        assert game_simple.best_step_count == float("inf")
        assert game_simple.total_best_distance == float("inf")
        
        # Assert player initial position
        assert game_simple.start_x == 0
        assert game_simple.start_y == TILE_SIZE * 2
        assert game_simple.x == 0
        assert game_simple.y == TILE_SIZE * 2
    
    def test_load_map(self, game_simple):
        assert game_simple.height == 4
        assert game_simple.width == 8
        assert len(game_simple.map) == 4
        assert len(game_simple.map[0]) == 8


    def test_replace_start_with_air(self, game_simple):
        assert game_simple.map[2][0] == "."
    
    def test_find_end_position(self, game_simple):
        assert np.array_equal(game_simple.end_pos, np.array([6, 2]))
...
class TestPhysics:
    ...
    def test_gravity(self, game_simple):
        game_simple.y = 0
        initial_vel_y = game_simple.vel_y
        game_simple.on_ground = False
        game_simple.update(MovementDirection.IDLE)
        assert game_simple.vel_y == initial_vel_y + GRAVITY
    
    def test_jump_sets_velocity(self, game_simple):
        game_simple.on_ground = True
        game_simple.update(MovementDirection.JUMP)
        
        assert game_simple.vel_y == JUMP_STRENGTH + GRAVITY
        assert game_simple.on_ground is False
    
    def test_cannot_jump_when_not_on_ground(self, game_simple):
        game_simple.on_ground = False
        game_simple.update(MovementDirection.JUMP)
        
        assert game_simple.vel_y != JUMP_STRENGTH
    
    def test_horizontal_movement_left(self, game_simple):
        game_simple.x = TILE_SIZE * 2
        game_simple.y = TILE_SIZE 
        initial_x = game_simple.x
        game_simple.update(MovementDirection.LEFT)
        assert game_simple.x < initial_x
    ...
    def test_max_falling_speed_limit(self, game_simple):
        game_simple.vel_y = MAX_FALLING_SPEED + 10
        game_simple.update(MovementDirection.IDLE)
        
        assert game_simple.vel_y <= MAX_FALLING_SPEED
    
    def test_velocity_sliding_effect(self, game_simple):
        game_simple.vel_x = 2.0
        game_simple.update(MovementDirection.IDLE)
        
        assert 0 <= game_simple.vel_x < 2.0
...
class TestGameState:
    ...
    def test_get_state_values(self, game_simple):
        game_simple.vel_x = MOVE_SPEED
        game_simple.vel_y = 3.0
        game_simple.on_ground = True
    ...
    def test_get_state_offset_calculation(self, game_simple):
        game_simple.x = TILE_SIZE * 2 + TILE_SIZE * 0.3
        state = game_simple.get_state(size=1)
        metadata = state[-1]
        assert metadata[0] == 0
        
        game_simple.x = TILE_SIZE * 2 + TILE_SIZE * 0.7
        state = game_simple.get_state(size=1)
        metadata = state[-1]
        assert metadata[0] == 1
...
class TestStepAndReward:
    ...
    def test_completion_reward(self, game_simple):
        for _ in range(100):
            ...
            if done and game_simple.game_completed:
                assert reward >= 500
                break
    
    def test_distance_progress(self, game_simple):
        initial_distance = game_simple.best_distance
        for _ in range(10):
            game_simple.step(2)
            if game_simple.best_distance < initial_distance:
                break
    
    def test_no_progress_penalty(self, game_simple):
        game_simple.steps_since_progress = 174
...
class TestRestart:
    ...
    def test_restart_resets_position(self, game_simple):
        game_simple.x = TILE_SIZE * 5
        game_simple.y = TILE_SIZE * 1
        
        game_simple.restart_game()
        
        assert game_simple.x == game_simple.start_x
        assert game_simple.y == game_simple.start_y
    ...
    def test_restart_resets_velocities(self, game_simple):
        ...
        assert game_simple.vel_x == 0.0
        assert game_simple.vel_y == 0.0
    
    def test_restart_resets_game_state(self, game_simple):
        game_simple.game_completed = True
        game_simple.game_over = True
        game_simple.steps = 100
        game_simple.coins_collected = 5
        
        game_simple.restart_game()
        
        assert game_simple.game_completed is False
        assert game_simple.game_over is False
        assert game_simple.steps == 0
        assert game_simple.coins_collected == 0
    
    def test_restart_reloads_map(self, game_simple):
        game_simple.map[2][1] = "X"
        
        game_simple.restart_game()
        
        assert game_simple.map[2][1] == "."
